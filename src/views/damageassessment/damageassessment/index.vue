  ...
  <div>
    <div id="cesiumContainer" class="situation_cesiumContainer">
      ...
      <div class="button showPanel" v-if="!eqPanel.isHistoryEqPanelShow && isShow"
           @click="eqPanel.isHistoryEqPanelShow=true, isShow=false">
        展开专题详情
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    removeTab(tabName, index) {
      const tabInfo = this.transferTab(tabName);

      // 检查 transferTab 是否返回了有效的数组
      if (!tabInfo || tabInfo.length < 2) {
        console.error(`Invalid tab info for tab: ${tabName}`);
        return; // 直接返回，防止后续操作报错
      }

      const panel = tabInfo[0];
      const info = tabInfo[1];
      ...
      if (this.currentTab === this.tabs[index]) {
        ...
        if (this.tabs[index - 1]) {
          ...
          if (nextTab) this.eqPanel[nextTab] = true; // 添加判断
        } else {
          this.currentTab = this.tabs[index + 1];
          const nextTab = this.transferTab(this.currentTab)[0];
          if (nextTab) this.eqPanel[nextTab] = true; // 添加判断
    ...
    toggleYaanLayer() {
      ...
      if (yaanRegionLayer) {
        ...
        if (this.eqThemes.isshowRegion) {
          this.addRegionLabels(); // 添加区域标签
        } else {
          this.removeRegionLabels(); // 移除区域标签
        }
      }
    },
    // 添加区域标签
...
// 移除区域标签
    ...
    showPersonalCasualty() {
      const tabName = "人员伤亡";
      const isShowing = this.eqThemes.isshowPersonalCasualty;

      // 切换面板显示状态
      this.eqThemes.isshowPersonalCasualty = !isShowing;
      this.eqPanel.isPersonalCasualtyPanelShow = !this.eqPanel.isPersonalCasualtyPanelShow;

      // 切换人员伤亡图层显示状态
      let casualtyLayer = window.viewer.dataSources.getByName("YaanRegionLayer")[0];
      if (casualtyLayer) {
        casualtyLayer.show = !isShowing; // 根据当前状态切换显示
        this.eqThemes.isshowRegion = !this.eqThemes.isshowRegion;
        casualtyLayer.show = this.eqThemes.isshowRegion; // 根据 isshowRegion 的值显示或隐藏图层

        if (this.eqThemes.isshowRegion) {
          this.addCasualtyLabels(); // 添加区域标签
        } else {
          this.removeCasualtyLabels(); // 移除区域标签
        }
    }
      ...
      this.features.forEach(item => {
        ...
        if (center && center.length === 2) {
          ...
          let regionlabel = viewer.entities.add(new Cesium.Entity({
            position: position,
            label: new Cesium.LabelGraphics({
              text: `${item.name}: ${item.num}`, // 显示区域名称和人数
              scale: 1,
              font: '18px Sans-serif',
              style: Cesium.LabelStyle.FILL_AND_OUTLINE,
              outlineWidth: 2,
              verticalOrigin: Cesium.VerticalOrigin.CENTER,
              horizontalOrigin: Cesium.HorizontalOrigin.CENTER,
              fillColor: Cesium.Color.RED, // 设置为红色
              pixelOffset: new Cesium.Cartesian2(0, 0)
          })
        }));
          this.CasualtyLabels.push(regionlabel);
        }
      });
    },

// 移除人员伤亡标签
    removeCasualtyLabels() {
      this.CasualtyLabels.forEach(entity => window.viewer.entities.remove(entity));
      this.CasualtyLabels = [];
    },
    ...
    hidden(hidden) {
      console.log("触发了隐藏")
      console.log(hidden)
      this.eqPanel.isHistoryEqPanelShow = hidden;
      // this.eqPanel.isBuildingDamagePanelShow = hidden;
      // this.eqPanel.isEconomicLossPanelShow = hidden;
      this.eqPanel.isPersonalCasualtyPanelShow = hidden;
      this.isShow = true;
    },
...
<!--    <div id="cesiumContainer" class="situation_cesiumContainer">-->
<!--      &lt;!&ndash;  小组件  &ndash;&gt;-->
<!--      &lt;!&ndash;    <div class="layers">&ndash;&gt;-->
<!--      &lt;!&ndash;      <div class="layer" title="雅安市行政区划" @click="toggleYaanLayer"><img src="../../../assets/images/DamageAssessment/yaanRegion.png"></div>&ndash;&gt;-->
<!--      &lt;!&ndash;    </div>&ndash;&gt;-->
...
<!--            <div style="height: 10px;background-color: #054576"></div>-->
<!--            <el-divider content-position="left">-->
<!--              &lt;!&ndash;            <img src="../../../assets/icons/TimeLine/收起展开箭头左.png" style="height: 15px; width: 15px;">&ndash;&gt;-->
<!--              地震信息-->
<!--            </el-divider>-->
<!--            <div style="padding: 1px 20px 10px 20px">-->
<!--              &lt;!&ndash; 显示选项卡内容 &ndash;&gt;-->
...
<!--      &lt;!&ndash; 底部面板(考虑代码差异性过大，设计成子组件形式) &ndash;&gt;-->
<!--      <div class="panel" v-if="eqPanel.isHistoryEqPanelShow">-->
<!--        <historyEqPanel-->
<!--            :historyEqData="historyEqData"-->
<!--            :selectedTabData="selectedTabData"-->
...
<!--        })-->
<!--      }-->
<!--      if (!this.eqThemes.isshowRegion) { //false-->
<!--        // this.RegionLabels-->
<!--        this.RegionLabels.forEach(entity => window.viewer.entities.remove(entity));-->
<!--        this.RegionLabels = []-->
<!--      }-->
...
<!--          console.log(this.yaanitemcasual);-->
<!--          console.log('预估伤亡总数',this.PersonalCasualtyNum); // 检查预估伤亡总数-->
<!--          console.log('地区伤亡情况',this.yaanitemcasual); // 检查地区伤亡情况-->
