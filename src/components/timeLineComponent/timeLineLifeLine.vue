...
const ecData = ref([]);
const currentlyBlackedOutVillages = ref(0) ;// 供电
const currentInterruptedVillages = ref(0) ;// 通信
const roadBlockVillage = ref(0) ;// 道路
const centralizedWaterProjectDamages = ref(0); // 供水
const eqid = ref("")

const getVillagesAll=ref([]);
const getSupplySituationListAll=ref([]);
...
function fetchData() {
  getVillages(eqid.value).then(res => {
    getVillagesAll.value=res
    // console.log("getVillages",res)
  ...
  getSupplySituationList(eqid.value).then(res => {
    getSupplySituationListAll.value=res
  ...
  setTimeout(()=>{
    filterRecords(props.currentTime)
    // update()
//     const initChart = () => {
//       echartsInstance = echarts.init(chart.value);
//       const pieData = ecData.value
...
function filterRecords(currentTime) {
  ...
  if (Array.isArray(getVillagesAll.value)) {
    ...
    if (filteredRecords.length > 0) {
      ...
      latestTime.value = formatDateTime(latestRecord.insertTime);
      latestTime.value = formatDateChina(latestTime.value)
      currentInterruptedVillages.value = latestRecord.currentInterruptedVillages;
      currentlyBlackedOutVillages.value = latestRecord.currentlyBlackedOutVillages;
      roadBlockVillage.value = latestRecord.roadBlockVillage;
  ...
  if (Array.isArray(getSupplySituationListAll.value)) {
    const filteredRecords1 = getSupplySituationListAll.value.filter(record => new Date(record.insertTime) < new Date(currentTime));
    if (filteredRecords1.length > 0) {
      let latestRecord1 = filteredRecords1.reduce((max, record) => (new Date(record.insertTime) > new Date(max.insertTime) ? record : max));
      centralizedWaterProjectDamages.value = latestRecord1.reduce((sum, item) => sum + item.centralizedWaterProjectDamage, 0);
      // updateChart();
    } else {
...
//   bindListen(echartsInstance)
  ...
  //   series: ecData.value.map(data => ({
  //     ...data,
  //     parametricEquation: getParametricEquation(
  //         data.pieData.startRatio,
  //         data.pieData.endRatio,
  //         false,
  //         false,
  //         data.pieStatus.k,
  //         data.value
